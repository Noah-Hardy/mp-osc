"""
Pose processing module with GPU and CPU implementa                        if is_apple_silicon:
                            print("🍎 Apple Silicon detected - GPU buffer format issues prevent GPU use")
                            print("🔄 Using CPU delegate for MediaPipe Tasks stability")ons.
"""
import os
import time
import json
...
class PoseProcessor:
        ...
        try:
            ...
            if model_path and os.path.exists(model_path):
                ...
                try:
                    ...
                    # Always try GPU delegate first, regardless of platform
                    try:
                        ...
                        print(f"⚠️  GPU delegate failed: {gpu_error}")
                        if is_macos:
                            print("� macOS detected - falling back to CPU delegate for MediaPipe Tasks stability")
                        else:
                            print("🔄 Falling back to CPU delegate")
                        base_options = python.BaseOptions(
                            model_asset_path=model_path,
                ...
                # Create pose landmarker with explicit error handling
                try:
                ...
                except Exception as landmarker_error:
                    print(f"❌ Failed to create pose landmarker: {landmarker_error}")
    ...
    def setup_cpu_processor(self):
        ...
        # Warn if num_poses > 1 since CPU mode only supports 1 pose
        if mp_config.get('num_poses', 1) > 1:
            print("⚠️  CPU mode only supports single pose detection. num_poses setting will be ignored.")
...
class GPUPoseProcessor(PoseProcessor):
    ...
    def process_frame(self, frame, pose_landmarker, backend_name):
        ...
        try:
            ...
            try:
                # Import mediapipe for Image class (different from tasks API)
                import mediapipe as mp
            ...
            if pose_detected:
                ...
                for i, pose_landmark in enumerate(pose_landmarker_result.pose_landmarks):
                ...
                if (hasattr(pose_landmarker_result, 'pose_world_landmarks') and 
                    ...
                    for i, pose_world_landmark in enumerate(pose_landmarker_result.pose_world_landmarks):
                ...
                self.send_multiple_bounds_data(
                    ...
                    pose_landmarker_result.pose_world_landmarks if all_pose_world_landmarks else None
                )
                
                self.osc_sender.send_message("/mp/status", json.dumps({"status": len(pose_landmarker_result.pose_landmarks)}))
                
                # Draw all pose landmarks
                for pose_landmark in pose_landmarker_result.pose_landmarks:
                    self._draw_gpu_landmarks(image, pose_landmark)
            else:
